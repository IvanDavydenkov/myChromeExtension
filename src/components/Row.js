...
const Row = (prop) => {
  return (
    <li
      className={Math.floor(prop.buy_usd - prop.sell_usd) > averageVolume ? "ex_list__item isInteresting" : "ex_list__item"}
      id={Math.floor(prop.buy_usd - prop.sell_usd) > averageVolume ? "ex_list__item isInteresting" : "ex_list__item"}>
      <p className="ex_list__txt" id="ex_list__txt">{`${normalizeTime(prop.start_time)}`}</p>
      <p
        className="ex_list__txt" id="ex_list__txt">{Math.floor(prop.buy_usd - prop.sell_usd)}</p>
